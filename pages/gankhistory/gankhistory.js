...
Page({
  ...
  onLoad: function (options) {
    this.setData({ type: options.type || 'all', pageNo: 1 });
    wx.setNavigationBarTitle({
      title: options.type || '全部干货'
    });
    wx.showLoading({
      title: '加载中',
    });
  ...
  queryHistory(loadmore) {
    getGankHistory({ type: this.data.type, pageNo: this.data.pageNo }).then(res => {
      wx.hideLoading();
      let historyList = res.results;
      if (this.data.type == '福利'){
        historyList = res.results.map(item=>{
          item.images = [];
          item.images.push(item.url);
          return item;
        });
      }
